...
export class EmailValidatorService {
  ...
  constructor(
    private options: ValidationOptions = { check_mx: true, check_smtp: false },
  ) {}
    ...
    try {
      ...
      return {
        ...
        checks: {
          ...
          smtp: this.options.check_smtp
            ? await this.checkSMTP(domainInfo)
            : undefined,
        },
    ...
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          ...
          API_CONFIG.TIMEOUTS.DNS_LOOKUP_MS,
        );
      });
  ...
  private determineDeliverability(
