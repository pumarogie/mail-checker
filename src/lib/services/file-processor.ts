...
export class FileProcessorService {
    ...
    try {
      ...
      return {
        ...
        processing_stats: {
          ...
          truncated: fileResult.emails.length === FILE_LIMITS.MAX_EMAILS_FREE_TIER,
        },
      };
  ...
  private validateFileType(file: File): SupportedFileType {
    ...
    throw createFileError('format', 'Please upload Excel (.xlsx, .xls) or CSV files.');
  }

  private async extractEmails(buffer: Buffer, fileType: SupportedFileType): Promise<string[]> {
    switch (fileType) {
      case 'xlsx':
      case 'xls':
      case 'csv':
  ...
  private extractEmailsFromSpreadsheet(buffer: Buffer): string[] {
    try {
      ...
      throw createFileError('extraction', 
